...
export default function Login() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(t('auth.loginFailed', { defaultValue: 'Login failed' }));
    }
  }
    ...
    <div className="min-h-screen flex items-start justify-center px-4 py-6 relative overflow-hidden bg-background sm:items-center sm:py-10">
      ...
      >
        ...
        <form className="space-y-5" onSubmit={submit}>
          ...
          >
            {t('auth.login', { defaultValue: 'Login' })} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </button>
        </form>
